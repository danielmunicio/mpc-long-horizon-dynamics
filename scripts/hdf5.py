...
def csv_to_hdf5(args, data_path):
    ...
    hdf5(data_path + 'train/', 'train.h5',  args.attitude,   args.position, args.history_length)
    hdf5(data_path + 'valid/', 'valid.h5',  args.attitude,   args.position, args.history_length)
    hdf5(data_path + 'test/',  'test.h5',   args.attitude,   args.position, args.history_length)

def hdf5(data_path, hdf5_file, attitude, position, history_length):
            ...
            if history_length == 0:
                ...
                if args.model_type == "mlp":
                    ...
                    for i in range(num_samples):
                        ...
                        Y[i,:] = data_np[i+history_length,:data_np.shape[1] - 4]
                else:
                    X = np.zeros((num_samples, history_length, data_np.shape[1]))
                    Y = np.zeros((num_samples, data_np.shape[1] - 4))
