...
def csv_to_hdf5(args, data_path):
    ...
    hdf5_v2(data_path + 'train/', 'train.h5',  args.attitude,  args.history_length, args.unroll_length)
    hdf5_v2(data_path + 'valid/', 'valid.h5',  args.attitude,  args.history_length, args.unroll_length)
    hdf5_v2(data_path + 'test/',  'test.h5',   args.attitude,  args.history_length, args.unroll_length)
...
def hdf5_v2(data_path, hdf5_file, attitude, history_length, unroll_length):
    ...
    for file in tqdm(os.listdir(data_path)):
        if file.endswith(".csv"):
            ...
            linear_acceleration_data = data[['ax', 'ay', 'az']].values
            angular_acceleration_data = data[['ang_acc_x', 'ang_acc_y', 'ang_acc_z']].values
            control_data = data[['delta_e', 'delta_a', 'delta_r', 'delta_t']].values


            # Concatenate all data
            data_np = np.hstack((linear_acceleration_data, attitude_data, angular_acceleration_data, control_data))
